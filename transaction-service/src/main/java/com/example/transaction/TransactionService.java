...
import com.example.transaction.client.NotificationServiceClient;
...
public class TransactionService {
    ...
    protected void processSaga(Transaction tx) {
        ...
        try {
            ...
            accountServiceClient.lockFunds(tx.getFromAccount(), amount, tx.getTransactionRef(),
            ...
            accountServiceClient.credit(tx.getToAccount(), amount);
            ...
            accountServiceClient.releaseLock(tx.getFromAccount());
            ...
            try {
                notificationServiceClient.sendTransactionCompleted(
                        "demo@example.com", // In production, get from account/customer service
    ...
    protected void compensate(Transaction tx, BigDecimal amount, Exception cause) {
        ...
        try {
            log.debug("Unlocking funds for compensation: transactionRef={}, fromAccount={}, amount={}", 
                    tx.getTransactionRef(), tx.getFromAccount(), amount);
            accountServiceClient.unlockFunds(tx.getFromAccount(), amount);
    ...
    public Transaction reverse(String transactionRef) {
        ...
        try {
            // Reverse: debit destination account, credit source account
            // No need to lock because the account already has the funds (from original credit)
            // and debit operation will check for sufficient balance
            
            log.debug("Step 1: Debiting destination account: transactionRef={}, toAccount={}, amount={}", 
                    transactionRef, tx.getToAccount(), amount);
            accountServiceClient.debit(tx.getToAccount(), amount);
            
            log.debug("Step 2: Crediting source account: transactionRef={}, fromAccount={}, amount={}", 
                    transactionRef, tx.getFromAccount(), amount);
            accountServiceClient.credit(tx.getFromAccount(), amount);

            tx.setStatus(Transaction.Status.FAILED);
            tx.setFailureReason(messageSourceService.getMessage("transaction.reversed.by.user"));
            tx.setUpdatedAt(OffsetDateTime.now());
            transactionRepository.save(tx);
            log.info("Transaction reversed successfully: transactionRef={}, fromAccount={}, toAccount={}, amount={}", 
